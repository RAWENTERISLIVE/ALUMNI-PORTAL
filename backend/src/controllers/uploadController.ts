...
export const serveFile = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  // path.basename() returns the last portion of a path, stripping any directory components
  const sanitizedFilename = path.basename(filename.replace(/\\/g, '/'));
  const uploadDir = path.resolve(__dirname, '../../uploads');
  const filePath = path.join(uploadDir, sanitizedFilename);

  // Security check: Ensure the file exists, is a file, and is within the uploads directory
