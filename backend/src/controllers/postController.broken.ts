...
// Helper to check if ID is in array by string comparison (avoids type issues)
const isIdInArray = (idArray: mongoose.Types.ObjectId[], idToCheck: string | mongoose.Types.ObjectId): boolean => {
  ...
  return idArray.some(id => id.toString() === idToCheckStr);
};
import mongoose from 'mongoose';
...
export const likePost = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (existingReactionIndex !== -1 && post.reactions) {
...
export const toggleFeaturePost = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (req.user.role !== 'admin') {
      res.status(403).json({ success: false, message: 'Only administrators can feature posts' });
      return;
    }
...
export const bookmarkPost = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (alreadyBookmarked) {
      ...
      await post.save();
      res.status(200).json({ success: true, message: 'Post unbookmarked successfully', isBookmarked: false });
    } else {
      // Add the bookmark
      post.bookmarks.push(userId);
      await post.save();
      res.status(200).json({ success: true, message: 'Post bookmarked successfully', isBookmarked: true });
    }
  } catch (error: any) {
...
export const voteOnPoll = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!post.pollOptions || post.pollOptions.length <= optionIndex) {
      res.status(400).json({ success: false, message: 'Invalid poll option' });
      return;
    }
    ...
    if (post.pollOptions && post.pollOptions[optionIndex]) {
      ...
      const voteId = typeof userId === 'string' ? new mongoose.Types.ObjectId(userId) : userId;
      post.pollOptions[optionIndex].votes.push(voteId);
    }
...
export const sharePost = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { originalPostId, content, visibility, shareType = 'share' } = req.body;
    if (!req.user || !req.user._id) {
      res.status(401).json({ success: false, message: 'Unauthorized' });
      return;
    }
...
export const getUserTimeline = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const formattedPosts = posts.map(post => ({
      ...
      isLiked: post.reactions ? post.reactions.some((reaction: any) => 
        reaction.userId.toString() === userId.toString() && reaction.type === 'like'
      ) : false,
      isBookmarked: post.bookmarks ? post.bookmarks.includes(userId) : false
    }));
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch user timeline', error: error.message });
  }
};
...
export const getBookmarkedPosts = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    const formattedPosts = posts.map(post => ({
      ...
      isLiked: post.reactions ? post.reactions.some((reaction: any) => 
        reaction.user.toString() === userId.toString() && reaction.type === 'like'
      ) : false,
  ...
  try {
    ...
    const posts = await Post.find({
      $or: [
        ...
        { author: { $in: connections }, visibility: { $in: ['public', 'connections'] } },
        // User's own posts
        { author: userId }
      ]
    })
